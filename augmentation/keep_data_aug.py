...
from augmentation import Cutout as C
from augmentation import  trivialaugment, GridMask


class Cutout_Keep(object):
    ...
    def __call__(self, images, m, threshold):
        ...
        for (i,img) in enumerate(images_):
            ...
            while(True):
                ...
                if m[i][y1: y2, x1: x2].sum()  <= threshold[i] or threshold[i] == 0:
                    mask[y1: y2, x1: x2] = 0.
                    break
                # if m[i][y1: y2, x1: x2].mean()  <= threshold[i]/self.length**2 or threshold[i] == 0:
                #     mask[y1: y2, x1: x2] = 0.
        ...
        return images.cuda()
class Cutout_Keep_fast_2(object):
    def __init__(self,length = 16, mean=[x / 255.0 for x in [125.3, 123.0, 113.9]], std=[x / 255.0 for x in [63.0, 62.1, 66.7]], early=False):
    ...
    def __call__(self, images, candidate):
        images_ = images.clone().detach()
        h = images.size(2)
        w = images.size(3)
...
class HaS_Keep_fast(object):
    def __init__(self, length, early=False):
        self.length = length
        self.early = early

    def __call__(self, ori_images, images, importance, threshold):
        # images: (batch_size, channel, h, w)
        # importance: (batch_size, h, w) 每个像素的重要性
        # threshold: (batch_size, ,1) 每张图片对应的阈值
        images_ = images.clone().detach()
        ...
        for i,img in enumerate(images_):
            ...
            while(True):
                ...
                if importance[i][y1: y2, x1: x2].sum() >= threshold[i]:
        ...
        for (i,img) in enumerate(images_):
            ...
            while(True):
                ...
                if importance[i][y1: y2, x1: x2].sum()  <= threshold[i] or threshold[i] == 0:
                    ...
                    break
            images_[i] = img
        return images_.cuda(), rand_index.cuda()
     
class GridMask_Keep_fast(object):
    def __init__(self, length ):
        # d1和d2决定格子的间隙和宽度
        # d是d1和d2之间的随机数，d为格子+间隙的长度，格子的大小为d*ratio
        self.length = length
    ...
    def __call__(self,ori_images, images, importance, threshold):
        ...
        images_ = images.clone().detach()
        # GridMask是一个batch的操作
        # grid = GridMask(d1=self.d1, d2=self.d2, mode=1)
        h = images.size(2)
        w = images.size(3)
        # images_ = grid(images_)
        for i,img in enumerate(images_):
            mask_zero = torch.Tensor(np.zeros(img.shape, np.float32))
            while(True):
                y = np.random.randint(h)
...
class GridMask_Keep_fast_2(object):
    def __init__(self, length ):
        # d1和d2决定格子的间隙和宽度
        # d是d1和d2之间的随机数，d为格子+间隙的长度，格子的大小为d*ratio
        self.length = length
...
class Randaugment_Keep_fast_2(object):
...
class Randaugment_Keep_fast(object):
    def __init__(self, length, N, M, early=False):
        self.length = length
        self.n = N
        self.m = M
...
class FastAugmentation_Keep_fast(object):
    def __init__(self, length, early=False):
...
class FastAugmentation_Keep_fast_2(object):
    def __init__(self, length, early=False):
        self.length = length
        self.early = early
        self.cutout = C(1,length)
...
class Autoaugment_Keep_fast(object):
    def __init__(self,  length,   early=False):
        self.length = length
        self.cutout = C(1,length)
...
class Autoaugment_Keep_fast_2(object):
    def __init__(self,  length,   early=False):
        self.length = length
        self.cutout = C(1,length)
...
class Autoaugment_Keep_fast_2_v2(object):
    def __init__(self,  length,   early=False):
        self.length = length
        # self.cutout = C(1,length)
        self.cutout = Cutout_Keep(length)

    def __call__(self, ori_images, images, candidate,mask_list, threshold_list):
        images_ = images.clone().detach()
        h = images.size(2)
        w = images.size(3)
        ...
        for i,img in enumerate(images_):
            ...
            mask_zero[:,y1: y2, x1: x2] = ori_images[i,:,y1: y2, x1: x2].clone().detach().cpu()
            img[:,y1: y2, x1: x2] = mask_zero[:,y1: y2, x1: x2]
            # img = self.cutout(img.cpu(), mask_list, threshold_list)
            images[i] = img
        images = self.cutout(images, mask_list, threshold_list)
        return images.cuda()

class Trivialaugment_keep_fast_2(object):
...
class Trivialaugment_keep_fast(object):
    def __init__(self, length=16):
        self.length = length
        self.cutout = C(1,length)
    def __call__(self,ori_images, images, importance, threshold):
        ...
        for i,img in enumerate(images_):
            ...
            #     # if importance[i][y1: y2, x1: x2].mean()  <= threshold[i]/256 or threshold[i] == 0:
            #     if importance[i][y1: y2, x1: x2].sum()  <= threshold[i] or threshold[i] == 0:
            #         mask[y1: y2, x1: x2] = 0.
            #         break
            # mask = torch.from_numpy(mask)
...
class Autoaugment_Keep(object):
    def __init__(self, train_transform, length, N, M, early=False):
        self.length = length
        self.n = N
        self.m = M
        self.early = early
...
# Fast Autoaugment
class FastAugmentation_Keep(object):
    def __init__(self, length, early=False):
        self.policies = fa_reduced_cifar10()
        self.length = length
        self.early = early
    ...
    def __call__(self, images, importance, threshold):
        ...
        for i,img in enumerate(images_):
            ...
            # img = self.fast(img)
            # img = self.tensor(img)
            img = self.after_transform(img.cpu())
            img[:,y1: y2, x1: x2] = mask_zero[:,y1: y2, x1: x2]
            # img = self.cutout(img)
...
# RandomErasing
class RandomErasing_Keep(object):
...
# TrivialAugment
class Trivialaugment_keep(object):
    def __init__(self, length=16):
        self.length = length
...
class SubPolicy_keepAA(object):
    def __init__(self, p1, operation1, magnitude_idx1, p2, operation2, magnitude_idx2, M, fillcolor=(128, 128, 128)):
        ...
        # (0.1, "invert", 7, 0.2, "contrast", 6, M, fillcolor),
        self.p1 = p1
        self.operation1 = func[operation1]
        # maxval1 = max(ranges[operation1][-1], ranges[operation1][0])
        # minval1 = min(ranges[operation1][-1], ranges[operation1][0])
        index1 = min(int(M * len(ranges[operation1])), len(ranges[operation1]) - 1)
        self.magnitude1 = ranges[operation1][index1]

        self.p2 = p2
        self.operation2 = func[operation2]
        # maxval2 = max(ranges[operation2][-1], ranges[operation2][0])
        # minval2 = min(ranges[operation2][-1], ranges[operation2][0])
        index2 = min(int(M * len(ranges[operation2])), len(ranges[operation2]) - 1)
        self.magnitude2 = ranges[operation2][index2]
